...
class DINOTracker():
    ...
    def get_sampler(self):        
        train_fg_trajectories, train_bg_trajectories = self.load_trajectories()
        train_sampler = DinoTrackerSampler(
            fg_trajectories=train_fg_trajectories,
            bg_trajectories=train_bg_trajectories,
            fg_traj_ratio=self.config["fg_traj_ratio"],
            batch_size=self.config["train_batch_size"],
            range_normalizer=self.range_normalizer,
            ...
            num_frames=self.config["batch_n_frames"],
            keep_in_cpu=self.config['keep_traj_in_cpu'],
            pseudo_labels_path=self.config.get("pseudo_labels_path", None),
            video_resw=self.config["video_resw"],
            video_resh=self.config["video_resh"]
        )
        return train_sampler
    ...
    def get_model(self):
        ...
        # Checkpoint logic
        detected_iter = get_last_ckpt_iter(self.ckpt_folder)
        yaml_init_iter = self.config.get("init_iter", 0)

        # Use the smaller of detected or YAML, or force YAML
        self.init_iter = yaml_init_iter

        if self.init_iter > 0:
            model.load_weights(self.init_iter) # DEBUG this
    ...
    def train_setup(self):
        ...
        # Freeze delta_dino and tracker_head
        for param in model.delta_dino.parameters():
            param.requires_grad = False
        for param in model.tracker_head.parameters():
            param.requires_grad = False

        # Only GNN parameters are trainable
        params = [{"params": model.gnn.parameters(), "lr": self.config["lr_gnn"]}]
    ...
    def get_inputs_and_labels(self, sampler):
        """
        Fetch sample from sampler and prepare full input dict for model,
        including adjacency matrix if pseudo labels are present.
        """
        sample = sampler()

        # Labels are always the target points normalized, shape [B, 2]
        labels = sample["t2_points_normalized"][:, :-1]

        # Start building input dict
        inputs = {
            "t1_points": sample["t1_points"],
        ...
        # If pseudo labels exist, add them and build adj_matrix
        if "pseudo_labels" in sample:
            pseudo_labels = sample["pseudo_labels"]

            # Build fully connected adjacency matrix without self-loops
            B = pseudo_labels.shape[0]
            adj_matrix = torch.ones(B, B, device=pseudo_labels.device) - torch.eye(B, device=pseudo_labels.device)

            inputs["pseudo_labels"] = pseudo_labels
            inputs["adj_matrix"] = adj_matrix
    ...
    def train(self):
        ...
        sampler_batch_iterations = self.config.get("sampler_batch_iterations", 100_000)
        ...
        for i in tqdm(range(self.init_iter, total_iterations)):
            ...
            coords = model(inputs) # calling the model

            tracking_loss = self.of_loss_fn(coords, labels).mean()
            loss = tracking_loss

            # Optional GNN pseudo label loss
            if "pseudo_labels" in inputs and "adj_matrix" in inputs:
                pseudo_labels = inputs["pseudo_labels"].to(coords.device)
                if pseudo_labels.shape[0] <= coords.shape[0]:
                    selected_coords = coords[:pseudo_labels.shape[0]]
                    gnn_loss = torch.nn.functional.mse_loss(selected_coords, pseudo_labels)
                    # loss += gnn_loss
                else:
                    print("⚠️ Skipping GNN loss: not enough coords to match pseudo labels")

            # Optional additional losses
            if i >= self.config.get("apply_cyc_after", 0):
                consistent_track_loss = self.get_cycle_consistency_loss(model, (
                    inputs["t1_points"], inputs["source_frame_indices"], inputs["target_frame_indices"], inputs["frames_set_t"]
                ))
                loss += self.config["lambda_cyc"] * consistent_track_loss
            else:
                consistent_track_loss = torch.tensor(0.0)

            if i >= self.config.get("apply_cl_ref_after", 0):
                loss_cl_refiner = self.get_refiner_contrastive_loss(model, frames_set_t=inputs["frames_set_t"])
                loss += self.config["lambda_cl_ref_bb"] * loss_cl_refiner
            else:
                loss_cl_refiner = torch.tensor(0.0)

            loss_cl_dino_bb = self.get_dino_bb_contrastive_loss(model, frames_set_t=inputs["frames_set_t"])
