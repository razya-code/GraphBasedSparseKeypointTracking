...
#     print(f"Foreground tracks: {is_fg.sum()}")
#     print(f"Track coordinates range: x=[{coords[:, 0].min()}-{coords[:, 0].max()}], y=[{coords[:, 1].min()}-{coords[:, 1].max()}]")
#     print(f"Segmentation mask shape: {segm_mask.shape}, non-zero pixels: {segm_mask.sum()}")
    
#     # If no foreground points found, use all points
#     if is_fg.sum() == 0:
#         print("Warning: No foreground points found, using all points for visualization")
...
def run(args):
    ...
    segm_mask_path = sorted(list(Path(masks_path).glob("*.jpg")) + list(Path(masks_path).glob("*.png")))[args.vis_start_frame]
    bg_of_trajectories_path = config_paths["bg_trajectories_file"]
    trajs_path = os.path.join(config_paths["grid_trajectories_dir"], f"keypoint_trajectories.npy")
    occ_path = os.path.join(config_paths["grid_occlusions_dir"], f"keypoint_occlusions.npy")
    model_vis_dir = config_paths['model_vis_dir']
    ...
    os.makedirs(model_vis_dir, exist_ok=True)
    dotted_vis_name = f"dotted_keypoints_fps_{args.fps}.mp4" if args.erosion_kernel_size is None else f"dotted_tracks_erosion_kernel_{args.erosion_kernel_size}_fps_{args.fps}.mp4"
    tracks_video_no_trace = viz_utils.plot_tracks_v2(video, tracks[is_fg], occluded[is_fg], rainbow_colors=True, point_size=args.point_size)
    print(tracks_video_no_trace.shape, video.shape, tracks[is_fg].shape, occluded[is_fg].shape)
    save_video(tracks_video_no_trace, os.path.join(model_vis_dir, dotted_vis_name), fps=args.fps)
