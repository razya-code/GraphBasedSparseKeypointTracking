...
#             x_s, y_s = global_scale_point(x_norm, y_norm, scale=1.1)  # Slight global scale
#             x = int(x_s * width)
#             y = int(y_s * height)
#             pts.append((x, y))
...
# print("🎉 All done! Points are now larger, edges more transparent, and colors vivid & consistent.")
...
for frame_idx, frame_file in enumerate(frame_files):
    ...
    if query is not None:
        ...
        pts = []
        for (x_norm, y_norm) in keypoints:
            x_s, y_s = global_scale_point(x_norm, y_norm, scale=1.1)  # optional global scale
...
print(f"✅ Video created: {output_video}")
print("🎉 All done! Points are now correctly aligned to the horse.")
